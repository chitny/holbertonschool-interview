...
{
	size_t binary_tree_size = 0;

	if (tree == NULL)
		return (0);
	if (tree->left != NULL)
		binary_tree_size = binary_tree_size + binary_tree_size(tree->left);
	if (tree->right != NULL)
		binary_tree_size = binary_tree_size + binary_tree_size(tree->right);
...
/**
 * heap_check_father - checks if father value is greater than the child
